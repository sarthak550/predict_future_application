...
- [Sprint 27 complete — Engagement & Retention](project_sprint27_complete.md) — S27-T1 (Big Call) and S27-T2 (Probability Chart) both done 2026-05-09; chart is pure View segments, fire-and-forget outside tx, CRON_SECRET guard confirmed
- [Missing category in CATEGORIES array — feature silently unreachable](feedback_missing_category_in_array.md) — S32-T3: FlagshipEventSection gated on category==="FINANCE" but FINANCE not in CATEGORIES picker array; tsc does not catch this
- [Computed field omitted from API return shape](feedback_return_shape_missing_computed_field.md) — S32-T4: expertCount computed inside helper but not forwarded in results.map(); optional type in ApiFlagshipEvent means tsc passes silently
- [Non-member data leak via Prisma spread](feedback_non_member_data_leak.md) — S56-T5: group detail spreads full Prisma object (including memberships roster) to non-members when partially opening access for RTJ/OPEN groups
