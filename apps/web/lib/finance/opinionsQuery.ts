...
export function parseOpinionsFilters(searchParams: Record<string, string | string[] | undefined>): OpinionsFilters {
  ...
  // Default = ALL calls (founder 2026-08-17, reverting the short-lived
  // graded-first default: pending calls are shareable now, so the original
  // "no share affordances on page one" motivation no longer applies).
