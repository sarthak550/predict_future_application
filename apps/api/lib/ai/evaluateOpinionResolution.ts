...
const PARSE_SYSTEM_PROMPT = `You are an Indian-equity-market research analyst. Given an analyst quote, direction tag, and publish date, extract the primary tradeable instrument and determine the exact calendar date when this call should be evaluated.
...
const VERDICT_SYSTEM_PROMPT = `You are an Indian-equity-market research analyst evaluating whether an analyst's price call came true.
...
- Keep reasoning to exactly one sentence. Start with the instrument name and its price change.`;
...
async function callGemini<T>(
  ...
  try {
    ...
    console.warn(`[evaluateOpinionResolution] Gemini network error: ${err instanceof Error ? err.message : err}`);
    return null;
  }
  ...
  if (!response.ok) {
    if (response.status === 429) {
      ...
      console.warn(`[evaluateOpinionResolution] Gemini returned ${response.status}`);
    }
    return null;
  }
  ...
  const text = (data as Record<string, unknown>)?.candidates?.[0 as never] as Record<string, unknown> | undefined;
  const part = ((text?.content as Record<string, unknown> | undefined)?.parts as unknown[])?.[0] as Record<string, unknown> | undefined;
  const content = part?.text as string | undefined;
  if (!content) return null;
...
export async function parseOpinionTimeframe(opinion: {
  ...
  const parseUserMessage = `Analyst quote: "${opinion.quote}"
Direction tag: ${opinion.direction}
Published date: ${opinion.publishedAt.toISOString().slice(0, 10)}
Article headline (for context): "${opinion.headline}"
...
Identify the primary tradeable instrument and assess whether this call can be resolved against price data.`;
  ...
  const parsed = validateParseResult(rawParse, opinion.publishedAt);
  if (parsed === null) {
 ...
 * @param opinion.publishedAt  - When the opinion was published (start of price window).
...
export async function evaluateOpinionResolution(opinion: {
  ...
  // If the caller already has the window from DB, we still need Pass 1 for instrument/ticker/claim.
  // We always run Pass 1 so we can get the full ParseResult.
...
Identify the primary tradeable instrument and assess whether this call can be resolved against price data.`;
  ...
  const effectiveWindowDays = opinion.resolutionWindowDays ?? parsed.impliedWindowDays;
  ...
  const priceWindow = await getPriceWindow(parsed.ticker, opinion.publishedAt, effectiveWindowDays);

  if (!priceWindow) {
    const note = `Price data unavailable for ${parsed.instrument}`;
    console.log(`${logPrefix} NOT_GRADED: ${note}`);
    return {
      status: "NOT_GRADED",
  ...
  const priceDataStr =
    `${parsed.instrument}: ₹${priceAtCall.toFixed(2)} → ₹${priceAtResolution.toFixed(2)} ` +
  ...
  const verdictUserMessage = `Original quote: "${opinion.quote}"
Direction tag: ${opinion.direction}
Specific claim extracted: "${parsed.specificClaim}"
