...
| S55-T8 | HIGH | ✅ done | Compact CommunitiesList replaces spotlight + rail in My Groups Discover (~52px rows) |
...
**S58 queued (was original S57):** Discovery polish — cover image upload, per-group notification prefs (unlock for safe fan-out at scale), category landing pages, unified search, featured curation, GroupModerationAction audit table, bulk approve/reject.

---

## Sprint 57

**Theme:** Member self-service — Leave group + Transfer ownership + Archive. Closes the gap S54 left when it shipped the kick-out without the walk-away. Preserves the OWNER-exactly-one invariant we rely on across ban/approve/reject.
**Status:** COMPLETE (5/5 done) — QA PASS 2026-06-07
**Brief:** `.claude/agent-memory/ceo-product-strategist/cto_assignment_brief_sprint57.md`

| ID | Pri | Status | Title |
|---|---|---|---|
| S57-T1 | CRIT | ✅ done | API: POST /api/groups/[id]/leave (self-leave; 409 if caller is OWNER) |
| S57-T2 | CRIT | ✅ done | API: POST /api/groups/[id]/transfer-ownership (atomic tx, outgoing owner → ADMIN) |
| S57-T3 | CRIT | ✅ done | API: POST /api/groups/[id]/archive (isArchived=true; verify discover/getUserGroups filters) |
| S57-T4 | HIGH | ✅ done | Mobile: replace handleLeave stub with confirmation + Transfer/Archive kebab actions for OWNER |
...
**S60 queued:** Category landing pages + audit-table archival policy (if growth becomes a concern) + S59 QA followups.

---

## Sprint 59

**Theme:** Make OPEN groups feel alive. Group market creation push (the engagement unlock — S58 pipes filled), GroupModerationAction audit table (proper moderation history surface), AsyncStorage → User.defaultGroupNotificationLevel migration, bulk approve/reject (operator inbox sanity), featured flag (editorial lever).
**Status:** IN PROGRESS
**Brief:** `.claude/agent-memory/ceo-product-strategist/cto_assignment_brief_sprint59.md`

| ID | Pri | Status | Title |
|---|---|---|---|
| S59-T0 | CRIT | ✅ done | Schema: GroupModerationAction + User.defaultGroupNotificationLevel + Group.isFeatured (single migration) |
| S59-T1 | CRIT | ✅ done | Audit write helper + backfill across 8 routes |
| S59-T2 | HIGH | ✅ done | User notif default API + mobile AsyncStorage→server one-way migration (code clean; runtime needs npm run dev not next dev — predev hook exists) |
| S59-T3 | CRIT | ✅ done | Group market created push (third helper) + markets/create call site |
| S59-T4 | HIGH | ✅ done | Bulk approve/reject endpoints + mobile multi-select inbox |
| S59-T5 | MED | ✅ done | Featured flag: admin route + discover sort + mobile star badge |

**Sprint-level invariants:**
- ONE schema migration (T0) folds THREE schema changes (audit model + user default field + group featured flag). Honors `serialize_schema_writes` because they go in the same migration.
- `prisma generate` MUST run before dev server restart per `prisma_generate_after_migration` (S54+S56+S58 all hit this).
- 8 routes need `logModerationAction` calls (ban/unban/remove/leave/transfer/archive/approve/reject) — easy to miss one. QA audits.
...
|---|---|---|---|
| S68-T1 | HIGH | ✅ done (QA pass, incl. runtime) | Points balance (Profile + bet panel), My Bets view (new positions endpoint + Markets chip + deep-links), watchlist "More" → Saved |

_Restored the spendable points balance (S66 regression) on the Profile + bet panel; added a "My Bets" surface (dedicated /api/profile/me/positions endpoint since profile/me caps at 10) reachable from a Markets-tab chip and a Profile "See all my bets" link; watchlist "More" now deep-links to the Saved filter._

## Sprint 67

| ID | Priority | Status | Title |
|---|---|---|---|
| S67-T1 | HIGH | ✅ done (QA pass 8/8) | Analyst tier credential: accuracy gate → lifetime net-PnL gate (≥+200/+1000/+4000) |

_Analyst tiers now reward conviction/stakes (lifetime net PnL) instead of accuracy %. Accuracy kept as a Track Record stat. Negative PnL → Rookie. Admin recalc route + one-time Profile notice for existing users (recalc-on-deploy)._

## Sprint 66

| ID | Pri | Status | Title |
|---|---|---|---|
| S66-T1 | CRIT | ✅ done (QA pass) | Profile: elegant header restyle (avatar, name, analyst headline) |
| S66-T2 | CRIT | ✅ done (QA pass) | Profile: stack all three tab sections as always-visible scroll |
| S66-T3 | HIGH | ✅ done (QA pass) | Profile: new Achievements section — quiet rows, no gamified emoji/pills |
| S66-T4 | HIGH | ✅ done (QA pass) | Profile: activity preview (capped 5 rows, interleaved) + markets inline |
| S66-T5 | MED | ✅ done (QA pass) | Profile: Track Record section visual polish + empty states |
...
| ID | Priority | Status | Title |
|---|---|---|---|
| S64-T1 | HIGH | ✅ done (QA pass) | Create-a-bet prefill: in-app story URL instead of raw sourceUrl |
| S64-T2 | HIGH | ✅ done (QA pass) | LinkifiedText: route predictfuture.app URLs in-app |

## Sprint 63

| ID | Priority | Status | Title |
|---|---|---|---|
| S63-T1 | HIGH | ✅ done (QA pass) | Create screen: add initialDescription prefill param |
| S63-T2 | HIGH | ✅ done (QA pass) | NewsFeedCard: "Create a bet" CTA (news → market deep-link) |
